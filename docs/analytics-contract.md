...
* Events from this SDK carry `source: "ios-sdk"`. The React Native and Flutter bridges emit no
  events of their own — everything they produce comes through this pipeline and is reported as
  `ios-sdk` too, so the collector cannot currently tell a wrapper app from a native one. See §8 of
  the contract for the additive-metadata proposal if you need to.
