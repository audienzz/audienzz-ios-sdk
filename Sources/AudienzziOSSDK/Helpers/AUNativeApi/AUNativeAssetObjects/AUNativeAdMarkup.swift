...
public class AUNativeAdMarkup: NSObject {
    ...
    init?(nativeAdMarkup: NativeAdMarkup?) {
        ...
        self.assets = nativeAdMarkup.assets?.compactMap {
            AUNativeAdMarkupAsset(adMarkup: $0)
        }
        ...
        self.eventtrackers = nativeAdMarkup.eventtrackers?.compactMap {
            AUNativeEventTrackerResponse(eventTracker: $0)
        }
    ...
    func unwrap() -> NativeAdMarkup {
        ...
        nativeAdMarkup.eventtrackers = self.eventtrackers?.compactMap {
            $0.unwrap()
        }
