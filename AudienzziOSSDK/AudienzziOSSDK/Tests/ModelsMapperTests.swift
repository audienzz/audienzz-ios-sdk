...
class ModelsMapperTests: XCTestCase {
    ...
    func testNetworkConvertingModel(by type: AUAdEventType) {
        ...
        XCTAssertNotNil(
            eventModel,
            "EnetModel \(type.rawValue) musn't be a nil"
        )
        XCTAssertTrue(
            eventModel?.type == type,
            "\(type.rawValue) model has incorrect type"
        )
    }
    ...
    func testConvertNetworkBidWinner() {
        ...
        let convertedToNetModel =
            convertToNetworkModel(dbModel) as! AUBidWinnerEvent

        XCTAssertEqual(
            convertedToNetModel.resultCode,
            eventModel?.resultCode,
            "Not Equal models"
        )
        XCTAssertEqual(
            convertedToNetModel.adUnitID,
            eventModel?.adUnitID,
            "Not Equal models"
        )
        XCTAssertEqual(
            convertedToNetModel.targetKeywords,
            eventModel?.targetKeywords,
            "Not Equal models"
        )
        XCTAssertEqual(
            convertedToNetModel.isAutorefresh,
            eventModel?.isAutorefresh,
            "Not Equal models"
        )
        XCTAssertEqual(
            convertedToNetModel.autorefreshTime,
            eventModel?.autorefreshTime,
            "Not Equal models"
        )
        XCTAssertEqual(
            convertedToNetModel.adViewId,
            eventModel?.adViewId,
            "Not Equal models"
        )
        XCTAssertEqual(
            convertedToNetModel.size,
            eventModel?.size,
            "Not Equal models"
        )
        XCTAssertEqual(
            convertedToNetModel.adType,
            eventModel?.adType,
            "Not Equal models"
        )
        XCTAssertEqual(
            convertedToNetModel.adSubType,
            eventModel?.adSubType,
            "Not Equal models"
        )
        XCTAssertEqual(
            convertedToNetModel.apiType,
            eventModel?.apiType,
            "Not Equal models"
        )
        XCTAssertEqual(
        ...
        XCTAssertEqual(
            convertedToNetModel.initialRefresh,
            eventModel?.initialRefresh,
            "Not Equal models"
        )
    }
    ...
    func testConvertNetworkAdClick() {
        ...
        let dbModel = AUEventDB(payload)
        let convertedToNetModel =
            convertToNetworkModel(dbModel) as! AUAdClickEvent
    ...
    func testConvertNetworkBidRequest() {
        ...
        let dbModel = AUEventDB(payload)
        let convertedToNetModel =
            convertToNetworkModel(dbModel) as! AUBidRequestEvent
    ...
    func testConvertNerworkAdCreation() {
        ...
        let dbModel = AUEventDB(payload)
        let convertedToNetModel =
            convertToNetworkModel(dbModel) as! AUAdCreationEvent
    ...
    func testConvertNerworkAdClose() {
        ...
        let dbModel = AUEventDB(payload)
        let convertedToNetModel =
            convertToNetworkModel(dbModel) as! AUCloseAdEvent
    ...
    func testConvertNerworkFailedLoad() {
        ...
        let dbModel = AUEventDB(payload)
        let convertedToNetModel =
            convertToNetworkModel(dbModel) as! AUFailedLoadEvent
        ...
        XCTAssertEqual(
            convertedToNetModel.errorCode,
            eventModel?.errorCode,
            "Not Equal models"
        )
        XCTAssertEqual(
            convertedToNetModel.errorMessage,
            eventModel?.errorMessage,
            "Not Equal models"
        )
    }
    ...
    func testConvertNetworkScreenImpression() {
        ...
        let dbModel = AUEventDB(payload)
        let convertedToNetModel =
            convertToNetworkModel(dbModel) as! AUScreenImpression
    ...
    private func convertToNetworkModel(_ event: AUEventDB) -> AUEventHandlerType
    {
        ...
        guard
            let netModel: AUEventHandlerType = convert(
                fromType: model.type,
                of: model
            )
        else {
    ...
    private func convert(fromType: AUAdEventType, of payload: PayloadModel)
        -> AUEventHandlerType?
    {
        switch fromType {
        case .BID_WINNER:
