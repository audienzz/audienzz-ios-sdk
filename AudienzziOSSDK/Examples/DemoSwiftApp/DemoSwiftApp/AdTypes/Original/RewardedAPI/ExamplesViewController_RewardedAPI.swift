...
extension ExamplesViewController {
    func createRewardedView() {
        ...
        rewardedView.frame = CGRect(origin: CGPoint(x: 0, y: getPositionY(lazyAdContainerView)), size: CGSize(width: view.frame.size.width, height: 300))
        rewardedView.backgroundColor = .magenta
        rewardedView.parameters = videoParameters
        
        lazyAdContainerView.addSubview(rewardedView)
        rewardedView.createAd(with: gamRequest, adUnitID: gamAdUnitVideoRewardedOriginal)
