...
extension ExamplesViewController {
    ...
    func createRenderingNativeView() {
        nativeRenderingView = AUNativeView(configId: nativeRenderingStoredImpression, isLazyLoad: false, adType: .rendering)
        nativeRenderingView.nativeParameter = nativeConfiguration()

        let gamRequest = GAMRequest()
        nativeRenderingView.createAd(with: gamRequest)
        
        nativeRenderingView.onNativeLoadRequest = { [weak self] request, targetingKeywords in
            ...
            AudienzzGAMUtils.shared.prepareRequest(gamRequest, bidTargeting: targetingKeywords)
        ...
        addConstrains(subView: exampleView, container: adContainerView, height: 200)
    ...
    func createRenderingNativeLazyView() {
        nativeLzyRenderingView = AUNativeView(configId: nativeRenderingStoredImpression, adType: .rendering)
        nativeLzyRenderingView.nativeParameter = nativeConfiguration()

        let gamRequest = GAMRequest()
        nativeLzyRenderingView.createAd(with: gamRequest)
        
        nativeLzyRenderingView.onNativeLoadRequest = { [weak self] request, keywords in
            guard let self = self else { return }
            guard let request = request as? GAMRequest else {
                print("Faild request unwrap")
                return
            ...
            self.adRenderingLazyLoader = GADAdLoader(adUnitID: gamRenderingNativeAdUnitId, rootViewController: self,
                                        adTypes: [GADAdLoaderAdType.customNative], options: [])
        ...
        let exampleView: ExampleNativeView = ExampleNativeView.fromNib()
        nativeLzyRenderingView.frame = CGRect(x: 0, y: getPositionY(lazyAdContainerView), width: self.view.frame.width, height: 200)
